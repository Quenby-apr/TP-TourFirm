            ...
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
            {
                ...
                foreach (var travel in info.TravelGuides)
                {
                    docBody.AppendChild(CreateParagraph(new WordParagraph
                    {
                        ...
                        (travel.TravelName + " :", new WordTextProperties {Bold = true, Size = "24", }
                            ) },
                        TextProperties = new WordTextProperties
                        {
                            Size = "24",
                    ...
                    foreach (var guide in info.TravelGuides[i].Guides)
                    {
                        ...
                        {
                            Texts = new List<(string, WordTextProperties)> {
                        (guide.Name + " " + guide.Surname + ". Место работы: \"" + guide.WorkPlace +"\"", new WordTextProperties { Bold = true, Size = "16", } ) },
                            TextProperties = new WordTextProperties
                            {
                                Size = "16",
                                JustificationValues = (DocumentFormat.OpenXml.Math.JustificationValues)JustificationValues.Both
